    ...
    if self.frame % self.CCP.STEER_STEP == 0:
      if self.CP.flags & (VolkswagenFlags.MEB | VolkswagenFlags.MQB_EVO):
        ...
        if CC.latActive:
          ...
          max_power = np.interp(CS.out.vEgo, [0., 0.5], [self.CCP.STEERING_POWER_MIN, min(self.steering_power_last + self.CCP.STEERING_POWER_STEP, self.CCP.STEERING_POWER_MAX)])
