    ...
    if self.CP.flags & VolkswagenFlags.DISABLE_RADAR and self.CP.openpilotLongitudinalControl:
      ...
      if self.frame % 50 == 0:
        if self.CP.flags & VolkswagenFlags.MEB:
          can_sends.append(CanData(0x17F00057, bytes.fromhex("20 00 00 00 FF FF 01 83"), self.CAN.pt)) # Radar Unknown (2 Hz) probably sufficient for MEB Gen 1,2
      ...
      if self.frame % 20 == 0:
        if self.CP.flags & VolkswagenFlags.MEB:
          can_sends.append(CanData(0x16A954AD, bytes.fromhex("00 80 02 10 FE 03 00 00"), self.CAN.pt)) # Radar Unknown (5 Hz) probably sufficient for MEB Gen 1,2
        elif self.CP.flags & VolkswagenFlags.MQB_EVO:
